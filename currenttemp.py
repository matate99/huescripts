...
tempnow = current['current']['feelslike_f']
highbroken = forecast['forecast']['forecastday'][0]['hour'][5]['maxtemp_f'] #high in F
...
for x in range (0, 12):
    ...
    for y in range (0, 2):
        b.set_light(x + 1, color)
    time.sleep(4)
    
for x in range (0, 2):
    revertlights(x + 1, lightsr[x])
